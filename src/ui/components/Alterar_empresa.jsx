...
function AlterarEmpresa({ empresa, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:3000/empresas/alterar/${empresa.id_empresa}`,
        {
          method: "PUT",
          headers: {
      ...
      if (data.success) {
        ...
        setMessage(`Erro ao alterar empresa: ${data.message}`);
      }
    } catch (error) {
      setMessage(`Erro ao conectar com o servidor: ${error.message}`);
    } finally {
