...
import "./Alterar_vendedores.css";

function AlterarVendedor({ vendedor, onClose, onSuccess }) {
  ...
  // Preencher o formulário com os dados do vendedor
  useEffect(() => {
    if (vendedor) {
      setFormData({
        id_usuario: vendedor.id_usuario || "",
        id_pessoa: vendedor.id_pessoa || "",
        taxa_comissao: vendedor.taxa_comissao || "",
        desconto_max: vendedor.desconto_max || "",
        ativo: vendedor.ativo === 1,
      });
    }
  }, [vendedor]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:3000/vendedores/vendedores/${vendedor.id_vendedor}`,
        {
          method: "PUT",
          headers: {
      ...
      if (data.success) {
        ...
        setMessage(`Erro ao atualizar vendedor: ${data.message}`);
      }
    } catch (error) {
      setMessage(`Erro ao conectar com o servidor: ${error.message}`);
    } finally {
